...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Update()
    {
        ...
        if (_lerp)
        {
            ...
            #region Taking "_currentPage" out to GalleryManager

            //GalleryManager.Instance.SetCurrentIndex(_currentPage);

            //Firing event
            if (_oldIndex != _currentPage)
            {
                //Debug.Log("Current page: " + _currentPage);

                OnIndexChanged_Event?.Invoke(this, new OnIndexChanged_EventArgs { index = _currentPage });

                //Debug.Log("Old and new are the not the same");
            }
            else
                //Debug.Log("Old: " + _oldIndex + " Current: " + _currentPage);

            _oldIndex = _currentPage;
    ...
    #region Functions to call from outside
    public void MoveToSelectedPage(int pageIndex)
    {
        LerpToPage(pageIndex);
    }

    public event EventHandler<OnIndexChanged_EventArgs> OnIndexChanged_Event;
    public class OnIndexChanged_EventArgs : EventArgs
    {
        public int index;
    }
